#!/usr/bin/python3
# ============================== HOW TO USE ============================== #
# just run the assets.py file from within the project folder. it will try
# to find a `.assetsignore` if there is any, and use it just like it works
# for git with `.gitignore`. it may take some time to finish.
# ======================================================================== #
...
# comments for the output file to separate user created code
...
# max of how far back to search for the project folder
...
project_folder = Path(".")
# log levels: LOG:1, WARNING:2, ERROR:4
log_level = 7 # 7 = 1 + 2 + 4
...
def log(msg, mtype:Level=Level.LOG, ignore_supress:bool=False):
    ...
    if (not ignore_supress):
        for level in list(Level)[::-1]:
            log_value -= level.value[1]

            if (log_value >= 0 and mtype == level): 
                should_log = True
