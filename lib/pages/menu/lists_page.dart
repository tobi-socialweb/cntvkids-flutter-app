...
class _ListsCardListState extends State<ListsCardList> {
  List<dynamic> lists = [];
  Future<List<dynamic>> _futureLists;

  ScrollController _controller;

  int currentPage;
  bool _continueLoadingPages;
  final int listsPerPage = 2;
  ...
  void initState() {
    ...
    currentPage = 1;
    _futureLists = fetchLists(currentPage);
  ...
  /// Fetch lists videos by page.
  ///
  /// Lists videos have category 10536 in their [categories].
  Future<List<dynamic>> fetchLists(int page) async {
    if (!this.mounted) return lists;

    /// Try get the requested data and wait.
    try {
      String requestUrl =
          "$LISTS_URL&categories[]=$LISTS_ID&page=$page&per_page=$listsPerPage";
      ...
      /// If request has succeeded.
      if (response.statusCode == 200) {
        /// Add new videos to [lists] by updating this widget's state.
        setState(() {
          lists.addAll(
              response.data.map((value) => Lists.fromJson(value)).toList());

          if (lists.length % listsPerPage != 0) _continueLoadingPages = false;
        });

        if (page == 1) _checkForForceUpdate(lists[0].id);
